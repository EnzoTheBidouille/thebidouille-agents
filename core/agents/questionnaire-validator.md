...
> (`ui_language`, `engine_format`) from the global `~/.claude/questionnaire.config.yaml` into your prompt.
...
3. `ui_language` and `engine_format` (facts from `~/.claude/questionnaire.config.yaml`, passed in the prompt).
