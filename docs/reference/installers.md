...
cost as three panels beside your sessions. It needs `npm i -g cohorte` — never `npx`, never a
shell, never an absolute path; the manifest names a bare binary and Francois resolves it on
`PATH` at spawn time.
